...
/**
 * Definition of an asynchronous spout channel, using the semantics
 * provided by <code>org.ect.reo.distengine.colouring.CC</code>.
 */
class AsyncSpout(prim:PrimID,logger:Logger) extends Primitive[SAT](logger) {
  val name = prim
  val endl = prim + "-l"
  val endr = prim + "-r"
  ...
  // not(a and b) and (a -> bsk) and (b -> ask) and
  // (not(a or b) -> ((as and not(bsk)) or (not(ask) and bsk)))
  val constr = new CNF( 
    List(List(-3,-1),List(-3,4),List(-1,2),List(3,1,-4,-2),List(3,1,-4,4),List(3,1,2,-2),
         List(3,1,2,4)),
    List(buid,ask,auid,bsk))
