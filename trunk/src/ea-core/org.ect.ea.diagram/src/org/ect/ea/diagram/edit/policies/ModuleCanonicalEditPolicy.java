...
import org.ect.ea.diagram.part.AutomataLinkDescriptor;
...
public class ModuleCanonicalEditPolicy extends CanonicalConnectionEditPolicy {
	...
	protected boolean shouldDeleteView(View view) {
		...
		if (view.getType() == null)
			return true;
		if (view.getElement().eContainer() == null)
			return true;
		return false;
	}
	...
	private Collection refreshConnections() {
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
			...
			int diagramLinkVisualID = AutomataVisualIDRegistry
			...
			for (Iterator LinkDescriptorsIterator = linkDescriptors.iterator(); LinkDescriptorsIterator
					.hasNext();) {
				AutomataLinkDescriptor nextLinkDescriptor = (AutomataLinkDescriptor) LinkDescriptorsIterator
						.next();
	...
	private Collection collectAllLinks(View view, Map domain2NotationMap) {
		...
		case ModuleEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(AutomataDiagramUpdater
		...
		case AutomatonEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(AutomataDiagramUpdater
						.getAutomaton_1001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection createConnections(Collection linkDescriptors,
		...
		for (Iterator linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator
				.hasNext();) {
			final AutomataLinkDescriptor nextLinkDescriptor = (AutomataLinkDescriptor) linkDescriptorsIterator
					.next();
